...
### Added
- **Opt-in savepoint semantics for nested transactions** (#141) — `TransactionBehaviorOptions.NestedSavepoints` (default off) makes each nested `[Transactional]` command run inside its own savepoint: pending changes are flushed and a savepoint is created before the nested command, and a failure rolls back to it — so an outer handler can catch the exception and commit without the inner command's writes. Uncaught failures still roll back the whole transaction. The previously dead `IUnitOfWork.CreateSavepointAsync`/`RollbackToSavepointAsync` members are now actually called (no-op implementations degrade to the join semantics), and `TransactionBehavior`'s docs no longer overpromise: by default nested dispatch **joins** the ambient transaction — a caught inner failure's writes commit with the outer work unless savepoints are enabled.
